...
interface TournamentScoreboardSectionProps {
  tournamentId: string
}

export function TournamentScoreboardSection({
  tournamentId,
}: TournamentScoreboardSectionProps) {
  ...
  if (rows.length === 0) {
    return <p className="empty-state">{t('matches.standings.empty')}</p>
  }

  return <ScoreboardTable rows={rows} />
}
