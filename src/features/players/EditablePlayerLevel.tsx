...
export function EditablePlayerLevel({
  ...
  const [level, setLevel] = useState<PlayerLevel>(
    player.self_selected_level as PlayerLevel,
  )
  ...
  if (!isEditable) {
    return (
      <span>
        {t(`level.${stats?.effective_level ?? player.self_selected_level}`)}
      </span>
    )
  }
  ...
  return (
    <span>
      ...
      <button
        ...
        onClick={() =>
          mutate({ id: player.id, updates: { self_selected_level: level } })
        }
      >
