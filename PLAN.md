...
6. **First commit.** _Test:_ `git log` shows one commit, tree clean.

## Phase 2 — Supabase Project & Schema

1. **Create new Supabase project** (MCP `create_project`, under the
...
4. **Migration: `tournament_participants`** — `tournament_id, player_id,
joined_at`, unique `(tournament_id, player_id)`. _Test:_ `execute_sql`
   rejects a duplicate insert.
5. **Migration: `matches` + `match_participants`** — `matches(id,
...
6. **Migration: `match_games`** — `match_id, game_number, team1_score,
team2_score`, unique `(match_id, game_number)`. _Test:_ `execute_sql`
   insert round-trip + unique constraint check.
7. **Views: `player_stats`** (total_matches, total_wins, win_rate,
...
2. **Players API** (list/create/update/getStats). _Test:_ integration
...
3. **Tournaments API** (create w/ computed cap, list, addParticipant,
...
4. **Matches/scoring API** (createMatch via an atomic RPC, recordMatchResult,
...
5. **TanStack Query hook conventions** (`usePlayers`, etc.), reused by all
...
1. **Player list view.** _Test:_ Playwright MCP against dev server with
   MCP-seeded players; confirm names/levels render.
2. **Create player form** (name, gender select, self-selected level select).
   _Test:_ RTL unit test (valid submit calls API; empty name blocked) +
   Playwright MCP round-trip verified via `execute_sql`.
3. **Edit player** (level field editable only while `total_matches < 3`).
   _Test:_ RTL test for both states; Playwright MCP edit-and-persist check.
4. **`getEffectiveLevel(player, stats)` pure helper** (mirrors the SQL view
...
3. **`selectCandidatePool(players, neededCount)`** — lowest match-count
...
4. **Singles: `pickSinglesPair(pool)`** — score all pairs by skill-gap;
...
5. **Doubles: `pickDoublesQuartet(pool)` + `splitIntoTeams(quartet)`** —
...
6. **`generateNextMatch(type, participants, pairingHistory)`** — composes
...
1. **`useDrawInputs(tournamentId)`** — assembles `CandidatePlayer[]` and
...
1. **`validateGameScore(score1, score2, {pointsPerGame, winBy, cap})`** —
   ...
   score (invalid).
2. **`validateMatchGames(games[], gamesPerMatch)`** — checks game count
...
3. **Result entry form** (dynamic per-game rows up to `gamesPerMatch`,
   ...
   payload.
4. **Persist + free the court** (mark `completed`, promote queued match to
...
1. **Standings table** reading `tournament_standings`, sorted games-won
...
2. **Refresh mechanism** — periodic poll (~30s) per the "no real-time"
...
3. **Edge cases** — empty tournament state; stable secondary sort (e.g. by
...
1. Confirm this is **view-driven** (Phase 2.7), not a batch job — every
...
2. **Cutover at exactly 3 matches.** _Test:_ integration test — 2 matches
   → still self-selected level; 3rd match recorded → `effective_level`
   switches to the win-rate-band value.
3. **UI reflects the switch without manual refresh** (query invalidation
...
1. **Setup** — `react-i18next`, `en.json`/`th.json`, toggle persisted to
   `localStorage`. _Test:_ unit test rendering under both locales.
2. **Extract all UI strings** built in Phases 4-9. _Test:_ grep-based check
   for leftover literal JSX strings; Playwright MCP toggle + screenshot
   comparison.
3. **Locale-aware date formatting** where relevant. _Test:_ unit test per
   locale.

## Phase 12 — Deployment to Vercel

1. **Push to GitHub** (no Vercel CLI/MCP available per `RESEARCH.md`, so
